...
export default function Methodology() {
  ...
  const heading = (
    <>
      ...
      <h2 className="font-display text-4xl text-text-primary md:text-7xl">
        Nossa <span className="italic accent-text">metodologia</span>
      </h2>
      <p className="mx-auto mt-4 max-w-md text-sm text-muted">
        Um caminho claro do diagnóstico ao crescimento — você sempre sabe em qual
        etapa o seu financeiro está.
      </p>
      <a
        href={BRAND.whatsapp}
        target="_blank"
  ...
  if (!isDesktop) {
    return (
      <section id="metodologia" className="bg-bg py-16">
        <div className="mx-auto max-w-md px-6 text-center">
          {heading}
          <div className="mt-10 grid grid-cols-2 gap-4">
            {METHOD.map((m) => (
              <MobileCard key={m.step} {...m} />
            ))}
          </div>
        </div>
      </section>
...
function DeskCard({ step, title, image }: { step: string; title: string; image: string }) {
  return (
    <motion.figure
    ...
    >
      ...
      <figcaption className="mt-3 text-center font-display text-xl text-text-primary">{title}</figcaption>
    </motion.figure>
  );
}

function MobileCard({ step, title, image }: { step: string; title: string; image: string }) {
  return (
    <motion.figure
    ...
    >
      <div className="relative aspect-square overflow-hidden rounded-2xl border border-stroke bg-surface">
        <img src={image} alt={title} loading="lazy" className="h-full w-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-t from-bg via-bg/20 to-transparent" />
        <span className="absolute left-3 top-2 font-display text-2xl italic accent-text">{step}</span>
      </div>
      <figcaption className="mt-2 text-center font-display text-base text-text-primary">{title}</figcaption>
    </motion.figure>
  );
}
